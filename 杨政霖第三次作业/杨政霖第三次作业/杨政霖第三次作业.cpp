...
//数字转图像
void Picture(int x, int i, int j)
{
	IMAGE img;
	if (x == 0)
	{
		loadimage(&img, _T("0.png"), 100, 100, true);
	}
	if (x == 2)
	{
		loadimage(&img, _T("2.png"), 100, 100, true);
	}
	else if (x == 4)
	{
		loadimage(&img, _T("4.png"), 100, 100, true);
	}
	else if (x == 8)
	{
		loadimage(&img, _T("8.png"), 100, 100, true);
	}
	else if (x == 16)
	{
		loadimage(&img, _T("16.png"), 100, 100, true);
	}
	else if (x == 32)
	{
		loadimage(&img, _T("32.png"), 100, 100, true);
	}
	else if (x == 64)
	{
		loadimage(&img, _T("64.png"), 100, 100, true);
	}
	else if (x == 128)
	{
		loadimage(&img, _T("128.png"), 100, 100, true);
	}
	else if (x == 256)
	{
		loadimage(&img, _T("256.png"), 100, 100, true);
	}
	else if (x == 512)
	{
		loadimage(&img, _T("512.png"), 100, 100, true);
	}
	else if (x == 1024)
	{
		loadimage(&img, _T("1024.png"), 100, 100, true);
	}
	else if (x == 2048)
	{
		loadimage(&img, _T("2048.png"), 100, 100, true);
	}
	putimage(10 * (j + 1) + j * 100, 10 * (i + 1) + i * 100, &img);//图片放置位置
}
...
int main()
{
	srand((unsigned int)time(NULL));//BUG修复，将时间种子种到主函数开头可以避免数字总是挨在一起

	initgraph(450, 450);	// 创建绘图窗口，大小为 450*450 像素
	setbkcolor(0x8C815E); cleardevice();//设置背景色
	int arr[ROW][ROW] = { 0 };
	Start(arr);
	Run(arr);
